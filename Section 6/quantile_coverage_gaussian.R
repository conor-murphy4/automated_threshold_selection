...
data_matrix <- read.csv("data/Gaussian.csv", row.names = 1, header=T)
...
for(ii in 1:num_samples){
  eqd_thresh <- eqd_gauss$thr[ii]
  num_excess <- eqd_gauss$len[ii]
  eqd_scale <- eqd_gauss$scale[ii]
  eqd_shape <- eqd_gauss$shape[ii]
...
saveRDS(A1_estimates_list, "output/coverage/Alg1_quantile_estimates_gauss.rds")
...
saveRDS(A2_estimates_list, "output/coverage/Alg2_quantile_estimates_gauss.rds")
...
A1_estimates_list <- readRDS("output/coverage/Alg1_quantile_estimates_gauss.rds")
A2_estimates_list <- readRDS("output/coverage/Alg2_quantile_estimates_gauss.rds")

true_quantiles <- qnorm(1-return_level_probs)

coverage_table <- list(3)
...
for(jj in 1:3){
  ...
  covered_A1 <- covered_A2 <- CI_widths_A1 <- CI_widths_A2 <- matrix(0,nrow=num_samples, ncol = length(return_level_probs))
  for (ii in 1:num_samples) {
    return_level_ests_A1 <- A1_estimates_list[[ii]]
    return_level_ests_A2 <- A2_estimates_list[[ii]]
    A1_ests_sorted <- apply(return_level_ests_A1, 2, sort)
    A2_ests_sorted <- apply(return_level_ests_A2, 2, sort)
    lower_limits_A1 <- A1_ests_sorted[(sig_level/2)*(num_boot),]
    lower_limits_A2 <- A2_ests_sorted[(sig_level/2)*(num_boot^2),]
    upper_limits_A1 <- A1_ests_sorted[(1-sig_level/2)*(num_boot),]
    upper_limits_A2 <- A2_ests_sorted[(1-sig_level/2)*(num_boot^2),]
  ...
  coverage_table[[jj]] <- as.data.frame(matrix(c(coverage_probs_A1, coverage_probs_A2, CI_ratios), nrow=3, ncol=length(return_level_probs), byrow=T))
}

print(coverage_table)

saveRDS(coverage_table, "output/tables/Table_S.20_coverage_probs_estimated_quantiles_gauss.rds")
