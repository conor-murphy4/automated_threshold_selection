...
for(ii in 1:num_samples){
  eqd_thresh <- eqd_gauss$thr[ii]
  eqd_num_excess <- eqd_gauss$len[ii]
  eqd_scale <- eqd_gauss$scale[ii]
  eqd_shape <- eqd_gauss$shape[ii]
...
saveRDS(A1b_estimates_list, "output/coverage/Alg1b_quantile_estimates_gauss.rds")
...
A1b_estimates_list <- readRDS("output/coverage/Alg1b_quantile_estimates_gauss.rds")
...
saveRDS(coverage_table, "output/tables/Table_S.23_coverage_probs_estimated_quantiles_gauss.rds")
